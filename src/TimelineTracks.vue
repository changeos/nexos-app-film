...
import { ratioPresetOf } from './flow/flowTypes'
...
// —— 输出分辨率小字（v0.1.37：项目 ratio → 六档预设合成分辨率；未知档隐藏）——
const outputRes = computed(() => {
  const p = ratioPresetOf(engine?.ratio.value ?? '')
...
/* ===================== 条本体（展开 ~180px / 折叠 28px；零 vh；
     v0.1.8 nx 刻面：卡面 0.5px 描边 + shadow-xs + divider 网格线） ===================== */
.tl-bar {
  display: flex;
...
.tl-toggle:hover { background: var(--nx-bg-hover); color: var(--nx-text-primary); }
.tl-toggle:focus-visible { outline: 2px solid var(--nx-ring-color); outline-offset: 1px; }
.tl-title {
...
.tl-range { width: 110px; height: 14px; accent-color: var(--nx-accent); }
.tl-btn {
  padding: 2px 8px;
...
.tl-btn:hover { background: var(--nx-bg-hover); color: var(--nx-text-primary); }
.tl-btn:active { background: var(--nx-bg-burn); }
.tl-btn:focus-visible { outline: 2px solid var(--nx-ring-color); outline-offset: 1px; }
...
.tl-heads {
  width: 96px;
  flex-shrink: 0;
  border-right: 0.5px solid var(--nx-line-divider);
  display: flex;
  flex-direction: column;
}
.tl-ruler-head { height: 18px; flex-shrink: 0; border-bottom: 0.5px solid var(--nx-line-divider); }
.tl-head-cell {
  display: flex;
  align-items: center;
...
.tl-tail {
  position: absolute;
  top: 3px;
  border-left: 1px dashed var(--nx-accent);
  color: var(--nx-accent);
  padding-left: 3px;
  white-space: nowrap;
}
...
/* ===================== 块通用（nx 刻面：badge 圆角 + 中性空槽底；
     功能色系保留——视频绿/配音蓝/BGM 紫/字幕琥珀 → token 四色） ===================== */
.tl-block {
  position: absolute;
  top: 3px;
  bottom: 3px;
...
/* 播放中当前段块（红色描边——与选中镜头的 accent 区分；两者同块叠加时红优先） */
.tl-block.playing {
  box-shadow: 0 0 0 2px var(--nx-danger);
  z-index: 1;
}
...
/* ===================== 播放头（v0.1.35；红=播放头功能色 → danger token） ===================== */
...
.tl-playhead.dragging .tl-ph-line {
  box-shadow: 0 0 0 0.5px color-mix(in srgb, var(--nx-danger) 45%, transparent);
}
.tl-ph-head {
  position: absolute;
  top: 0;
...
/* ① 视频块：分镜图背景（img 缩略）+ 镜头号角标；缺图=斜纹（success/warning 档） */
.tl-vblock {
  flex-direction: column;
  align-items: stretch;
...
.tl-vblock.ready {
  border: 1.5px solid var(--nx-success);
  background: var(--nx-success-bg);
}
.tl-vblock.semi { border: 1.5px solid var(--nx-warning); }
.tl-thumb {
  position: absolute;
  inset: 0;
...
.tl-vlabel {
  ...
  font-weight: 700;
  color: var(--nx-text-primary);
  background: color-mix(in srgb, var(--nx-bg-card) 80%, transparent);
  padding: 0 3px;
  border-radius: 3px;
  margin: 2px;
  white-space: nowrap;
