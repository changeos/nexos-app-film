...
//      左侧竖向选项卡栏（SideNav：顶部 🎬 FilmHub 回大厅 + 五流程阶段
//      ①剧情②分镜③定妆④音频⑤合成 + 工作台 + Hub 浏览（HubBrowse：项目
//      文件树 + 内容区 + 活动/成本/接入指南 Tab）+ 设置/成员；阶段徽章读
...
import { distVersionOf, distVersions, entryMtime, fileBasename, fmtBytes } from './flow/flowFiles';
...
// =============================================================================
// FilmHub 大厅 + 项目工作室（v0.1.1 两态）
// =============================================================================

/** 视图两态：list FilmHub 大厅（HubLobby 组件）/ studio 项目工作室。 */
...
async function loadWbTree(): Promise<void> {
  ...
  try {
    ...
    wbTree.value = Array.isArray(raw) ? raw : [];
  } catch {
...
<template>
  <div class="film-page">
    <!-- ==================== FilmHub 大厅（v0.1.1 显性大厅；HubLobby 组件） ==================== -->
    ...
    <template v-else>
      ...
      <div class="studio-shell">
        <SideNav :view="navView" :stage="flowStage" @select="setFlowView" @home="backToList" />
...
.ratio-card:hover:not(:disabled) { border-color: var(--accent, #E95420); }
.ratio-card.sel {
  border-color: var(--accent, #E95420);
  box-shadow: 0 0 0 1px var(--accent, #E95420) inset;
  background: var(--accent-soft, rgba(233, 84, 32, 0.06));
}
...
.ratio-bar {
  ...
  border-radius: 3px;
  background: var(--text-muted, #5E5C5F);
  opacity: 0.55;
}
