...
// v0.1.11 全局操作反馈：合成提交失败/转正成败 toast（合成任务走通知闭环）
...
async function downloadVersion(e: FilmFileEntry): Promise<void> {
  ...
  try {
    ...
    toast.error(t('film.actFailed') + (ctx ? ctx.errMsg(err) : String(err)))
  }
}
    ...
    <FlowPageHead stage="compose" :title="t('film.flowCompose')">
      <template #actions>
        ...
        <NxButton
          ...
          size="sm"
          :loading="composeBusy"
          :disabled="!ctx?.project.value || !ctx?.composeAvailable.value"
          :title="ctx?.isOffline.value ? t('film.capsOfflineTip') : t('film.finalEmpty')"
          @click="composeFinal"
        >{{ composeBusy ? t('film.taskRunning') + '…' : t('film.cpCompose') }}</NxButton>
      </template>
    </FlowPageHead>
...
<style scoped>
/* v0.1.8 nx 化：cache 半成品卡（dashed 槽位 + subtle 缩略内衬） */
...
.cp-cache-card:hover { border-color: var(--nx-line-strong); box-shadow: var(--nx-shadow-xs); }
.cp-cache-thumb {
  width: 100%;
  aspect-ratio: 16 / 10;
  border-radius: var(--nx-radius-input);
