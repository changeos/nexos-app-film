...
/** 工作室视图：五阶段页 + workbench 工作台 + hub 项目文件树浏览 + collab
 *  PR/Issues 协作（v0.1.10）+ models 项目模型设置（v0.1.38 八能力位默认源）
 *  + settings 设置/成员。 */
export type FlowView = FilmStage | 'workbench' | 'hub' | 'collab' | 'models' | 'settings'
...
/** 全部工作室视图（SideNav 选项卡集合 + 深链 view 参数合法域，v0.1.5）。
 *  hub 视图在导航列表不再单列（v0.1.5 删「Hub 浏览」项）——但视图本身保留：
 *  树卡「完整浏览」/ 树卡点文件 / 深链 view=hub 仍可达；collab 视图 v0.1.10
 *  新增（SideNav「协作」项 + 任务失败一键提 Issue 成功后跳转落点）。 */
export const FLOW_VIEWS: readonly FlowView[] = [
  ...FLOW_STAGES,
  'workbench',
  'hub',
  'collab',
...
/** 值是否为合法视图（深链 standalone view 参数校验；非法宽容忽略）。 */
export function isFlowView(v: string): v is FlowView {
  return (FLOW_VIEWS as readonly string[]).includes(v)
}
