...
// v0.1.37.1 初始定位：SideNav 树卡点击文件跳入（pendingHubFile）——挂载即
// 选中该文件并加载内容（注册表式：树纯导航，网页主区打开）。
// =============================================================================
...
// —— 初始定位（v0.1.37.1 注册表式）：SideNav 树卡点击文件跳入——pendingHubFile
//    置位即选中并加载该文件内容（树卡 selected 高亮 + 祖先目录自动展开在
//    HubTreeCard 内处理），消费后清空（同一项目重复点击同文件也生效）。
watch(
  () => ctx?.pendingHubFile.value ?? null,
  (path) => {
    if (!path || !projectId.value) return
    tab.value = 'files'
    void openFile(path)
    ctx!.pendingHubFile.value = null
  },
