...
//   - 画面区（stage）：按项目 ratio 适配（16:9 / 9:16 / 1:1），黑底 #000 圆角，
//     容器内居中 letterbox；顶行叠加镜头号徽章 + 时间码 mm:ss.f / 总时长；
//     底部字幕条（当前镜头 line 优先 desc，白字黑描边，CC 可开关）；
...
/** 画面比例 → stage class（16:9 / 9:16 / 1:1；缺省按 16:9 letterbox）。 */
const ratioClass = computed(() => {
  const r = engine?.ratio.value ?? '16:9'
  if (r === '9:16') return 'r-9-16'
  if (r === '1:1') return 'r-1-1'
  return 'r-16-9'
})
...
<template>
  <section class="card mon" :aria-label="t('film.previewMonitor')">
    ...
    <!-- 画面区：ratio 适配 + 黑底圆角 + 居中 letterbox；顶行徽章/时间码 + 底部字幕 -->
    <div class="mon-body">
      <div class="mon-stage-wrap">
        <div class="mon-stage" :class="ratioClass">
...
/* 宽画幅：宽撑满、高按比例（超高由 max-height 兜底 letterbox） */
.mon-stage.r-16-9 { aspect-ratio: 16 / 9; width: 100%; }
/* 竖画幅：高撑满、宽按比例（超宽由 max-width 兜底） */
.mon-stage.r-9-16 { aspect-ratio: 9 / 16; height: 100%; width: auto; }
/* 方画幅：高撑满（正方形两向等价） */
.mon-stage.r-1-1 { aspect-ratio: 1 / 1; height: 100%; width: auto; }
