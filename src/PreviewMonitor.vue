...
<style scoped>
/* ===================== 卡骨架（flex 纵列；零 vh；v0.1.8 nx token 化） ===================== */
.mon {
  flex: 1;
  min-height: 0;
  display: flex;
...
/* 模式徽章（「成片」态）：info soft 刻面（自含——不依赖宿主 .pill 作用域） */
.mon-mode {
  display: inline-flex;
  align-items: center;
  gap: 4px;
...
.mon-count { margin-left: auto; color: var(--nx-text-tertiary); }
.mon-body {
  flex: 1;
  min-height: 0;
  display: flex;
...
/* 画面区：黑底为媒体语义色（深浅主题恒黑）；圆角走 nx 卡片刻面 */
.mon-stage {
  position: relative;
  background: #000000;
  border-radius: var(--nx-radius-card);
...
.mon-video {
  ...
  height: 100%;
  object-fit: contain;
  background: #000000; /* 媒体底：恒黑（letterbox 补边） */
}
.mon-img {
  position: absolute;
  inset: 0;
...
/* 黑底占位（无图镜头：desc 大字）——画面叠加层：白字系为媒体语义色（恒定） */
.mon-ph {
  position: absolute;
  inset: 0;
  display: flex;
...
/* 顶行叠加：镜头号徽章 + 时间码（黑底半透明浮层——媒体叠加语义色） */
.mon-topline {
  position: absolute;
  top: 8px;
  left: 8px;
...
/* 底部字幕条（白字黑描边；line 优先 desc）——字幕渲染语义色恒定 */
.mon-cc {
  position: absolute;
  left: 10%;
  right: 10%;
...
/* —— 播放控制条（nx-btn secondary 刻面 + focus 环） —— */
.mon-controls {
  display: flex;
  align-items: center;
  gap: 6px;
...
.mon-btn:active:not(:disabled) { background: var(--nx-bg-burn); }
.mon-btn:disabled {
...
.mon-btn .mono { font-family: var(--nx-font-mono); font-weight: 700; font-size: 12px; }
/* 主播放钮：accent 实底 */
.mon-play {
  min-width: 42px;
