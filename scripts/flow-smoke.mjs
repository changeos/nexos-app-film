...
ok(r.nav.items.length === 9, `九选项卡渲染（🎬 FilmHub 回大厅 + 五阶段 + 工作台 + Hub 浏览 + 设置）`)
ok(r.nav.items[0].includes('FilmHub'), '品牌项：🎬 FilmHub（回大厅）')
...
ok(r.nav.items[6].includes('工作台') && r.nav.items[7].includes('Hub'), '工作台 + Hub 浏览选项卡')
ok(r.nav.items[8].includes('设置'), '设置/成员选项卡')
...
ok(r.nav.activeIndexAfterClick === 2, '点击「分镜」→ active 流转到第 3 项')
...
console.log('①.5 StoryPage 剧情页（文档管线：清理/分章/人物档案/转定妆）')
ok(r.story.sourceRows.length === 1 && r.story.sourceRows[0].includes('novel.txt'), '左栏源列表（novel.txt + 已清理/已分章徽章派生）')
ok(r.story.sourceRows[0].includes('已清理') && r.story.sourceRows[0].includes('已分章'), '源状态徽章：已清理 + 已分章')
ok(r.story.pipelineVisibleAfterSelect, '选中源 → 管线操作条出现（清理/分章/人物梳理）')
ok(r.story.cleanCall?.path.endsWith('/story/clean') && r.story.cleanCall.body?.source_file === 'sources/novel.txt', '「清理」→ POST story/clean（rules 带 source_file）')
...
ok(r.story.chapterRows.length === 2 && r.story.chapterRows[0].includes('第一章 风起') && r.story.chapterRows[0].includes('字'), '章节清单（index.json 派生 + 每章字数）')
ok(r.story.chapterTextShown, '点开章节 → ch-01.md 正文展示')
