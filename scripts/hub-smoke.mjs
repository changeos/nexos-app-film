...
//         ②b 初始定位（v0.1.37.1 注册表式）：pendingHubFile → 挂载即选中
//         加载 + 祖先展开；
//      ③ 定妆页集成：pendingCastSelect 消费 + 对象选中；
//      ④ SideNav 导航层级：顶部 🎬 FilmHub 回大厅 + v0.1.5「Hub 浏览」项已删
//         + 视图外链 ⧉（window.open 深链拼装）+ 树卡点击 file-click（完整
//         路径；迷你预览浮层已删）+ ④c 树卡原位大展开（上树 55% / 下内容
//         45% 三形态：文本/图片/音频 + 复杂内容「完整浏览」+ 收起恢复）；
//      ⑤ 纯函数：buildHubTree / hubFileIcon / hubTargetView / hubCastSelect /
//         deriveStageFromProject / isHubTextPath；⑤b 视图深链纯函数
//         （parseStandaloneQuery / buildStandaloneUrl round-trip）。
//
...
async function handle(method, path, body) {
  ...
  for (const pid of ['p1', 'p2', 'p3']) {
    ...
    if (pid === 'p1') {
      // v0.1.39.1 起按后端真形态返回 {root, files} 信封（树卡 hubFileEntries 归一）
      if (method === 'GET' && rest === 'files') return { root: '/tank/film/p1', files: TREE }
...
console.log('①b UI 重设计第一批（v0.1.7：卡片三态类名 / 表头筛选 popover / 空态骨架）')
ok(r.lobby.nx.cardInteractiveCount === 3, '卡片类名：hub-card + nx-card is-interactive（三态基座全挂载）')
ok(r.lobby.nx.cardSelectedByFocus, '卡片三态：focusin → is-selected 选中态类挂载（键盘可达）')
ok(r.lobby.nx.filterOpenWorks, '表头筛选 popover：点触发钮 → 面板开（Teleport body 的 .nx-pop__panel）')
ok(r.lobby.nx.filterCloseWorks, '表头筛选 popover：Esc → 面板关')
ok(r.lobby.nx.stageFiltered === 1, '阶段筛选收进列表头：定妆 → 命中 1 张（深海灯塔，产物启发式）')
ok(r.lobby.nx.stageReset === 3, '阶段筛选复位「全部」→ 3 张全回')
...
ok(r.browse.treeExpandAbsent, 'HubBrowse 左栏树卡无「⤢ 展开」钮（大展开为 SideNav 树卡形态专属）')
...
console.log('②b HubBrowse 初始定位（SideNav 树卡点击文件 → pendingHubFile）')
ok(r.locate.selectedRowPath === 'casting/props/长剑/card.md', '定位：目标文件行选中高亮（is-selected）')
ok(r.locate.ancestorExpanded, '定位：祖先目录自动展开（casting/props/长剑 → card.md 可见）')
ok(r.locate.contentText.includes('星海长剑'), '定位：挂载即加载内容（card.md 正文 pre）')
ok(r.locate.crumbsText.includes('card.md'), '定位：面包屑显示目标路径')
ok(r.locate.pendingConsumed, '定位：pendingHubFile 消费后清空')
...
console.log('④ SideNav 导航层级（v0.1.5：Hub 浏览项已删 + 视图外链 ⧉）')
ok(r.nav.homeText.includes('FilmHub'), '顶部 🎬 FilmHub 品牌项（回大厅）')
ok(r.nav.hubItemAbsent, '「Hub 浏览」导航项已删（与左下树卡重复；视图保留——树卡/深链仍可达）')
ok(r.nav.homeEmits === 1, '点品牌项 → home 事件（回大厅）')
ok(r.nav.extCount === 9, '九个视图项行尾 ⧉ 外链钮（五阶段+工作台+协作+设置+模型；FilmHub 除外）')
ok(
  r.nav.extOpens[0] === '/apps-assets/film/standalone.html?p=p1&view=storyboard',
  `⧉ 点击 → window.open 独立页深链（${r.nav.extOpens[0] || '无'}）`,
)
ok(r.nav.extClickNotSelect, '⧉ 点击不触发本行切页（stopPropagation——外链≠选卡）')
ok(r.nav.hubSelects.includes('hub'), 'hub 视图仍可达：树卡「完整浏览」→ select=hub')

console.log('④b SideNav 底部常开树卡（注册表式：渲染 / 全收 / 点击跳转）')
...
ok(
  r.nav.fileClicks.includes('story.md') && r.nav.fileClicks.includes('casting/characters/小明/front.png'),
  `点击文件 → file-click（完整路径）：${r.nav.fileClicks.join('、')}`,
)
ok(r.nav.popAbsent, '注册表式：迷你预览浮层已删（点击后无 .hub-mini-pop）')
ok(
  r.nav.browseEmits === 1 && r.nav.hubSelects.filter((v) => v === 'hub').length === 1,
  '「完整浏览」链接 → select=hub（hub 视图唯一导航入口=树卡；选项卡项已删）',
)

console.log('④c 树卡原位大展开（⤢：撑满左栏剩余全高 + 卡内下区三形态内容）')
ok(r.nav.expand.classesOn, '展开态类齐全：卡 is-expanded + 树卡容器/根 nav is-tree-expanded（flex 撑满剩余全高）')
ok(r.nav.expand.insideNav, '原位放大：卡片仍在 .fh-nav 内（无弹层/portal——宽度限左栏，绝不向右溢出）')
ok(r.nav.expand.previewPresent, '卡内下区内容预览区出现（上树 55% / 下内容 45% 分栏）')
ok(r.nav.expand.text.includes('月光落在月球快递站'), '下区形态①文本：story.md 等宽 pre（滚动）')
ok(r.nav.expand.clicksNotEmitted, '展开态点文件不再 emit file-click（下区加载，不跳 Hub 浏览）')
ok(r.nav.expand.img, '下区形态②图片：front.png data URL contain 直显')
ok(r.nav.expand.audio === 1, '下区形态③音频：bgm-001.mp3 audio 控件')
ok(r.nav.expand.complex, '复杂内容（视频/二进制）：提示走「完整浏览」（链接保留）')
ok(r.nav.expand.collapsedBack, '⤡ 收起钮恢复紧凑态（is-expanded / 下区内容区消失）')
...
// —— v0.1.7 大厅筛选纯函数（filterHubProjects / hubRatioOptions 同一代码路径）——
console.log('⑤c 大厅筛选纯函数（搜索 × 阶段 × 类别）')
{
  const fp = harness.filterHubProjects
  const P = (title, idea, ratio, script, artifacts) => ({
    title, idea, ratio, script, artifacts, id: title,
  })
  const list = [
    P('星际', '柯基快递', '16:9', [{ shot: 1 }], [{ name: 'final.mp4', bytes: 1 }]),
    P('灯塔', '守塔人', '9:16', [], [{ name: 'shot-1.png', bytes: 1 }]),
    P('草稿', '一个想法', '16:9', [], []),
  ]
  ok(fp(list, { q: '', stage: '', ratio: '' }).length === 3, 'filterHubProjects：不筛 → 全过')
  ok(fp(list, { q: '柯基', stage: '', ratio: '' }).length === 1, '搜索词（idea 命中）')
  ok(fp(list, { q: '', stage: 'compose', ratio: '' }).length === 1, '阶段筛（final.mp4 → compose）')
  ok(fp(list, { q: '', stage: '', ratio: '9:16' }).length === 1, '类别筛（9:16 竖版档）')
  ok(fp(list, { q: '灯塔', stage: 'casting', ratio: '9:16' }).length === 1, '三联交叠（搜索×阶段×类别）')
  ok(fp(list, { q: '', stage: '', ratio: '' }, ).length === 3 && JSON.stringify(harness.hubRatioOptions(list)) === JSON.stringify(['16:9', '9:16']), 'hubRatioOptions：分辨率档去重保序')
}

// —— 视图深链（v0.1.5）：parseStandaloneQuery / buildStandaloneUrl 同一代码路径 ——
console.log('⑤b 视图深链纯函数（standalone.html?p=&view=&cast=&file=）')
{
  const dl = harness.parseStandaloneQuery('?p=p1&view=storyboard')
  ok(dl.projectId === 'p1' && dl.view === 'storyboard' && !dl.cast && !dl.file, '基础形态：p + view 直达（storyboard）')
  const dlCast = harness.parseStandaloneQuery(
    '?p=p9&view=casting&cast=characters%2F' + encodeURIComponent('小明'),
  )
  ok(
    dlCast.cast?.type === 'characters' && dlCast.cast?.name === '小明',
    'casting 深链：&cast=<type>/<name> 对象定位（中文解码）',
  )
  const dlFile = harness.parseStandaloneQuery(
    '?p=p1&view=hub&file=' + encodeURIComponent('story/sources/novel.txt'),
  )
  ok(dlFile.view === 'hub' && dlFile.file === 'story/sources/novel.txt', 'hub 深链：&file=<path> 文件定位')
  ok(harness.parseStandaloneQuery('?view=story').projectId === '', '无 p → 大厅照旧（嵌入桌面壳地址栏无副作用）')
  ok(harness.parseStandaloneQuery('?p=p1&view=bogus').view === '', '非法 view 宽容忽略（退项目缺省页）')
  ok(
    harness.parseStandaloneQuery('?p=p1&view=casting&cast=bogus/小明').cast === null,
    '非法 cast 类型宽容忽略',
  )
  ok(
    harness.buildStandaloneUrl('p1', 'storyboard') === 'standalone.html?p=p1&view=storyboard',
    '外链拼装：standalone.html?p=<id>&view=<view>',
  )
  const built = harness.buildStandaloneUrl('p1', 'hub', { file: 'story/小说 第1章.md' })
  const rt = harness.parseStandaloneQuery(built.slice('standalone.html'.length))
  ok(
    rt.projectId === 'p1' && rt.view === 'hub' && rt.file === 'story/小说 第1章.md',
    '拼装 ↔ 解析 round-trip（中文/空格文件路径安全）',
  )
}
