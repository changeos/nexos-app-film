...
export async function run(): Promise<FlowSmokeResult> {
  ...
  // ============ ① SideNav：选项卡渲染 / 阶段徽章 / 点击流转 / 折叠 ============
  {
    ...
    // 点击「分镜」→ active 流转 + view 切换（items[0]=🎬 FilmHub 回大厅）
  ...
  // ============ ①.5 StoryPage：文档管线（源区/管线操作条/三 Tab/转定妆） ============
  {
    ...
    await flush(20)
    const chapterRows = Array.from(rightCard.querySelectorAll('.fh-row')).map((n) => (n.textContent ?? '').trim())
    const firstChapter = rightCard.querySelector('.fh-row') as HTMLElement
    firstChapter.dispatchEvent(new Event('click', { bubbles: true }))
    await flush(20)
    const chapterTextShown = (rightCard.querySelector('.fh-pre')?.textContent ?? '').includes('风起正文')
    // 人物档案 Tab：卡片 + 转定妆
    ...
    const toCastingBtn = rightCard.querySelector('.story-profile-card .fh-btn') as HTMLButtonElement
    toCastingBtn.click()
    await flush(20)
